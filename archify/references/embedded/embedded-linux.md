...
- `match()` 表示总线匹配；`probe()` 还要验证硬件支持并初始化。源码、设备树节点或模块文件不能单独证明 ready。
...
- “有设备树节点”不等于驱动已绑定；“已绑定”不等于 `probe()` 成功；“probe 成功”不等于用户态接口可用。
...
目标项目应固定 Linux kernel revision、设备描述最终内容、有效 `.config`、驱动构建清单、rootfs 内容、服务定义和必要运行记录。当前官方依据：Linux Driver Binding（https://docs.kernel.org/driver-api/driver-model/binding.html）与 Remote Processor Framework（https://docs.kernel.org/staging/remoteproc.html）。两页为滚动文档，不能替代目标版本源码核对。
