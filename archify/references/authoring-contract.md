...
- `componentType`: the seven legacy compatibility types `frontend`, `backend`, `database`, `cloud`, `security`, `messagebus`, `external`; the complete embedded role directory and profile context matrix are in [Embedded interface (A1)](../schemas/README.md#embedded-interface-a1).
...
`embedded-runtime` is opt-in and applies to all five diagram types; `deployment-ownership` remains Architecture-only. Its execution-entity requirements and supported context matrix are defined in the linked Schema section; all required context facts must be non-`unknown`. It requires evidence-backed execution ownership for applicable process/thread/task/isr nodes and explicit mechanisms for declared cross-domain or cross-context relationships; it does not prove scheduling, interrupt legality, DMA correctness, cache coherence, or real-time bounds. `linux-kernel` is kernel thread/process context and `linux-irq` is non-threaded interrupt context; threaded IRQ is not auto-labelled `linux-irq`. `software` may span multiple contexts, and bare-metal `runtime`/`version` identify firmware rather than an OS. Keep unknown facts visible and leave the profile unset when required facts are unavailable. Architecture comparisons containing non-empty embedded execution domains, execution-domain references, or required relations return structured `delta/embedded-context-unsupported` before output; never delete those facts to obtain a comparison.
