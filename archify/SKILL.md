...
1. If the request concerns embedded Linux, an RTOS, bare metal, or a mixed runtime, first bound the target-firmware evidence (version, board, build/config, startup, execution boundaries, and relevant artifacts). Determine Authoring Depth as `overview`, `mechanism`, or `source-interaction`, then make a lightweight inventory of entities, boundaries, relationships, evidence, and unknowns before authoring JSON. A broad request to organize an embedded subsystem defaults to `overview`; ask one question only when the missing answer kind would materially change the diagram type. Read [`references/embedded/depth-and-semantics.md`](references/embedded/depth-and-semantics.md) plus only the needed domain or ecosystem reference. Inspect the target repository read-only and write only the diagram or its evidence/delivery materials; missing build artifacts stay unknown. Do not default to building, downloading an SDK, launching a simulator, or accessing/writing a device; any extra action needs separate authorization.
2. Choose `architecture`, `workflow`, `sequence`, `dataflow`, or `lifecycle` from the question; an OS name alone never chooses the diagram type, and neither does a runtime name, hardware mechanism, or Authoring Depth.
...
4. Artifact first: the next tool action must write the candidate. Write the candidate before inspecting renderer internals. Do not plan exact coordinates in prose. Start with one clear main path, short side branches, and sparse labels. An overview has at most 12 primary nodes and no minimum; omit lower-priority detail for a mechanism view instead of merging independent hardware, execution, buffer/memory, or recovery entities. Mechanism and source-interaction views keep the entities and separate data movement, control, IRQ/notification, buffer handoff, completion, and recovery relationships needed by the question. Set `meta.quality_profile` to `"showcase"` unless the user explicitly requests a dense `standard` map. Start with automatic routes and labels. Do not add `via`, `channelX`, `channelY`, or `labelAt` before a diagnostic calls for one; apply at most one diagnosed geometry control per repair.
...
## Embedded Authoring Depth

Authoring Depth is an authoring-scope decision, not the Viewer Reading Depth `MAP` / `READ` / `FULL`. Use `overview` for a bounded structure map, `mechanism` for one question-specific behavior view, and `source-interaction` for the same five diagram types backed by bounded source/configuration evidence. If the user asks only for a mechanism or source interaction without saying whether structure, actions, order, data movement, or state matters, ask that one material question before authoring.

Across related diagrams, reuse a stable ID only for the same real entity and preserve its type, execution domain, and evidence-backed context. Reuse a relationship ID only for the same directed fact and mechanism. An overview may omit mechanism detail, but it must not reverse it, change a known mechanism, or collapse independent entities to meet its node budget. Use [`references/embedded/depth-and-semantics.md`](references/embedded/depth-and-semantics.md) for the inventory, view split, semantic roles, and completeness checklist.

## Embedded interface notes

When the target uses A1 embedded semantics, use only the existing approved shape: optional execution domains, execution context, mechanism, required relations, and `embedded-runtime` profile as documented in [`references/authoring-contract.md`](references/authoring-contract.md) and [`schemas/README.md`](schemas/README.md). Do not invent fields, force unknown entities, or enable the profile by default. The legacy seven component types remain a compatibility subset; the full embedded role directory adds only `software`, `process`, `thread`, `task`, `isr`, `hardware`, `buffer`, `memory`, and `bus`. `embedded-runtime` applies to all five diagram types; `deployment-ownership` remains Architecture-only. Architecture comparison involving non-empty execution-domain or required-relation context returns `delta/embedded-context-unsupported`; do not drop those facts to make a comparison pass. Other diagram types do not expand `--repo-root` repository evidence.
...
- Legacy component types are `frontend`, `backend`, `database`, `cloud`, `security`, `messagebus`, and `external`; embedded role additions are documented in `schemas/README.md`. Variants are `default`, `emphasis`, `security`, and `dashed`.
